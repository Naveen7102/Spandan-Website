...
public interface TeamMembersRepository extends JpaRepository<TeamMembers,Integer>{
    ...
    @Query(value = "select * from team_members t where t.sport_id=:sport_id and t.team_id=:team_id and t.participant_id=:participant_id", nativeQuery = true)
    TeamMembers getId(@Param("sport_id") Integer sport_id, @Param("team_id") Integer team_id, @Param("participant_id") Integer participant_id);
}
